...
app.get("/api/v3/tags", auth, (req, res, next) => {
...
app.route("/api/v3/messages").get(auth, (req, res, next) => {
    let filter = {};
    if (req.query.tags) {
...
app.delete('/api/v3/messages/:messageid', auth, ensureModeratorRole, (req, res, next) => {
    console.log("Delete request for message id: " + req.params.messageid);
    // req.params.messageid contains the :messageid URL component
    message.getModel().deleteOne({ _id: req.params.messageid }).then((q) => {
        if (q.deletedCount > 0)
...
app.get('/api/v3/users', auth, ensureModeratorRole, (req, res, next) => {
...
app.get("/api/v3/login", passport.authenticate('basic', { session: false }), (req, res, next) => {
    if (!req.user)
